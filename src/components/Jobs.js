...
export const Jobs = ({ filter, term, toggle }) => {
  const filteredJobs = jobs.filter((job) => {
    ...
    if (term === "") {
      ...
      if (
        (filter.length > 0 && filter.some((r) => job.tools.includes(r))) ||
        (filter.length > 0 && filter.some((r) => job.languages.includes(r)))
      ) {
        return job;
      } else return;
    } else if (
  ...
  const topMargin = () => {
    ...
    return toggle ? '100px' : '50px' 
    } else if (window.innerWidth >= 499 && window.innerWidth < 1000) {

        return toggle ? '150px' : '50px' 
    } else if (window.innerWidth < 400) {

        return toggle ? '250px' : '50px' 
    }

  }

  return (
    <JobContainer style={{ marginTop: topMargin() }}>
        ...
        return (
          <Job key={jobs.id}>
            <Info>
              ...
              <Tags>
                {jobs.languages.map((languages, j) => (
                  <Tag key={j}>{languages}</Tag>
                ))}
                {jobs.tools.map((tools, j) => (
                  <Tag key={j}>{tools}</Tag>
                ))}
              </Tags>
            </Info>
